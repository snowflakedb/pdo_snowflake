...
#define sf_free_s SF_MACRO_DEPRECATED_WARNING("sf_free_s is deprecated and it does nothing.")
    ...
    static inline char* sb_copy(void* dst, size_t dstsize, char const* src, long long srclen)
    {
        ...
        return (char*)sb_memcpy(dst, dstsize, src, copyLen);
    }

    // helper function for sb_strcat and sb_strncat
    static inline char* sb_cat(char* dst, size_t dstsize, char const* src, size_t srclen)
    {
        size_t dstlen = strlen(dst);
        return dstsize < dstlen ?
            NULL :
            sb_copy(dst + dstlen, dstsize - dstlen,
...
/// @brief Copy a string.
        ...
        const char* in_src)
    {
        ...
        return NULL;
#else
        return sb_copy(out_dest, in_destSize, in_src, -1);
#endif
    }
        ...
        size_t in_sizeToCopy)
    {
        ...
        return NULL;
#else
        return sb_copy(out_dest, in_destSize, in_src, in_sizeToCopy);
#endif
    }
        ...
        const char* in_src)
    {
        ...
        return NULL;
#else
        return sb_cat(out_dest, in_destSize, in_src, -1);
#endif
    }
        ...
        size_t in_sizeToCopy)
    {
        ...
        return NULL;
#else
        return sb_cat(out_dest, in_destSize, in_src, in_sizeToCopy);
#endif
    }
        ...
        ...)
    {
        ...
        const int ret = sb_vsnprintf(out_buffer, in_sizeOfBuffer, in_sizeOfBuffer - 1, in_format, ap);
        va_end(ap);
        ...
        ...)
    {
        va_list ap;
        va_start(ap, in_format);
        const int ret = sb_vsnprintf(out_buffer, in_sizeOfBuffer, in_sizeToWrite, in_format, ap);
        va_end(ap);
...
#define sb_sscanf sscanf
    ...
    /// character; -1 if the truncation or error occurred.
    static inline int sb_vfprintf(FILE* in_stream, const char* in_format, va_list in_argPtr)
    {
        return
#if defined(_WIN32) || defined(_WIN64)
        vfprintf_s(
    ...
    static inline int sb_fprintf(FILE* in_stream, const char* in_format, ...)
    {
        va_list ap;
        va_start(ap, in_format);
        const int ret = sb_vfprintf(in_stream, in_format, ap);
        va_end(ap);
    ...
    static inline int sb_printf(const char* in_format, ...)
    {
        va_list ap;
        va_start(ap, in_format);
        const int ret = sb_vfprintf(stdout, in_format, ap);
        va_end(ap);
